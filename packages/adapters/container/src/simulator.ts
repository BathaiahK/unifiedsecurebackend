...
export class ContainerSimulator {
  async scan(image: string): Promise<TrivyReport> {
    ...
    const misconfigurations = [
      {
        ...
        severity: 'HIGH' as const,
        resolution:
          'Add a RUN statement to create a non-root user and switch to it: RUN useradd -m nonroot && USER nonroot',
      },
      {
        type: 'config',
        id: 'CKV_DOCKER_14',
        ...
        description:
          'Docker HEALTHCHECK instruction not specified. Healthcheck can improve container reliability and orchestration',
        severity: 'LOW' as const,
        resolution:
          'Add HEALTHCHECK instruction to Dockerfile: HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 CMD curl -f http://localhost/ || exit 1',
      },
    ];
    ...
    const metadata: TrivyMetadata = {
      ...
      ImageID: randomUUID(),
      DiffIDs: ['sha256:e692418e3baf', 'sha256:3c3a4604a545', 'sha256:ec4b8955958d'],
      RepoTags: [image],
      ...
      ImageConfig: {
        history: [
          ...
          {
            created_by: 'RUN apt-get update && apt-get install -y curl libssl3 zlib1g',
            empty_layer: false,
          },
  ...
  generateReport(image: string, report: TrivyReport): ContainerScanReport {
    ...
    const layerDetails: LayerDetail[] = [
      ...
      },
      {
        ...
        diffId: 'sha256:abc1',
        sizeBytes: 89234567,
        command: 'RUN apt-get update && apt-get install -y curl libssl3 zlib1g',
        vulnerabilityCount: 3,
        secretCount: 0,
      },
      {
        index: 2,
        digest: 'sha256:ec4b8955958d',
        diffId: 'sha256:abc2',
        sizeBytes: 5234123,
        command: 'COPY app.js /app/',
        vulnerabilityCount: 0,
        secretCount: 0,
      },
      {
        index: 3,
        digest: 'sha256:aaa123456789',
        diffId: 'sha256:abc3',
        sizeBytes: 56234123,
        command: 'RUN npm install lodash jackson-databind log4j-core',
        vulnerabilityCount: 3,
        secretCount: 1,
      },
      {
        index: 4,
        digest: 'sha256:bbb456789abc',
        diffId: 'sha256:abc4',
        sizeBytes: 2123456,
        command: 'EXPOSE 8080',
        vulnerabilityCount: 0,
      ...
      {
        index: 7,
        digest: 'sha256:eee123456789',
        diffId: 'sha256:abc7',
        sizeBytes: 567890,
        command: 'CMD ["node", "app.js"]',
        vulnerabilityCount: 0,
        secretCount: 0,
      },
    ];
    ...
    const cisChecks: CisCheck[] = [
      {
        ...
        severity: 'HIGH',
        status: 'FAIL',
        remediation: 'RUN useradd -m appuser && USER appuser',
      },
      {
        id: 'DKR-0002',
        title: 'Use trusted base images',
        description: 'Always use official base images',
        severity: 'HIGH',
        status: 'PASS',
        remediation: 'Use debian:11-slim or alpine:latest',
      },
      {
        id: 'DKR-0003',
        title: 'Scan and fix known vulnerabilities',
        description: 'Scan for and fix known vulnerabilities',
        severity: 'HIGH',
        status: 'FAIL',
        remediation: 'Run trivy image and fix CVEs',
      },
      {
        id: 'DKR-0004',
        ...
        severity: 'MEDIUM',
        status: 'PASS',
        remediation: 'Replace ADD with COPY',
      },
      {
        id: 'DKR-0005',
        title: 'Set HEALTHCHECK',
        description: 'Docker HEALTHCHECK instruction not specified',
        severity: 'MEDIUM',
        status: 'FAIL',
        remediation: 'Add HEALTHCHECK --interval=30s CMD curl http://localhost:8080',
      },
      {
        id: 'DKR-0006',
        title: 'Avoid setuid/setgid bits',
        description: 'Avoid setuid/setgid bits in image',
        severity: 'LOW',
        status: 'PASS',
        remediation: 'Remove setuid/setgid bits from files',
      },
      {
        id: 'DKR-0007',
        title: 'Use read-only root filesystem',
      ...
      {
        id: 'DKR-0008',
        title: 'Bind to specific ports',
        description: 'Do not expose all ports',
        severity: 'LOW',
        status: 'PASS',
        remediation: 'Expose only required ports',
      },
    ];
