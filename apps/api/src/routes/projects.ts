...
export const projectsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/projects', async (req, reply) => {
    ...
    if (!body.success) {
      return reply
        .status(400)
        .send({ error: 'Invalid project data', details: body.error.flatten() });
    }
    try {
      ...
      if (e?.code === 11000 || e?.message?.includes('duplicate')) {
        return reply
          .status(409)
          .send({ error: `A project named "${body.data.name}" already exists` });
      }
      throw err;
    }
  });
...
function maskToken<T extends { gitToken?: string | null }>(
  project: T,
): T & { gitToken: string | null } {
